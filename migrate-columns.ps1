...
Write-Host ""
Write-Host "=== GMA Dashboard - Column Migration Verification ===" -ForegroundColor Cyan
Write-Host ""
...
# Step 2 - Try PATCH with priority + special_requests
Write-Host ""
Write-Host "[2] Testing PATCH with 'priority' and 'special_requests' columns..." -ForegroundColor Yellow
$patchHeaders = $headers.Clone()
$patchHeaders['Prefer'] = 'return=minimal'
$testBody = @{ priority = 'normal'; special_requests = @() } | ConvertTo-Json
...
try {
    ...
    Write-Host "    The dashboard will now persist priority and special requests." -ForegroundColor Green
} catch {
    ...
    Write-Host "      ADD COLUMN IF NOT EXISTS priority         text  DEFAULT 'normal'," -ForegroundColor White
    Write-Host "      ADD COLUMN IF NOT EXISTS special_requests jsonb DEFAULT '[]';" -ForegroundColor White
    Write-Host ""
